...
@IBDesignable open class RangeSeekSlider: UIControl {
    ...
    //values for custom UI
    private var sliderHeight = CGFloat(10)
    private var verticalLineWidth = CGFloat(2)
    private var verticalSmallLineHeight = CGFloat(8)
    private var verticalBigLineHeight = CGFloat(16)
    ...
    @IBInspectable open var minValue: CGFloat = 4.0 {
        didSet {
            refresh()
        }
    }
    ...
    @IBInspectable open var maxValue: CGFloat = 13 {
        didSet {
            refresh()
        }
    }
    ...
    @IBInspectable open var selectedMinValue: CGFloat = 4 {
        didSet {
    ...
    @IBInspectable open var selectedMaxValue: CGFloat = 13 {
        didSet {
    ...
    @IBInspectable open var step: CGFloat = 1.0
    ...
    private func drawVerticalLine(pos: CGFloat?,interval: Int) -> CAShapeLayer {
        var doYourPath = UIBezierPath()
        if interval % 2 == 0 {
            doYourPath = UIBezierPath(rect:
                CGRect(x: pos!, y: sliderLine.position.y + sliderHeight,
                       width: verticalLineWidth, height: verticalSmallLineHeight))
        } else {
            doYourPath = UIBezierPath(rect:
                CGRect(x: pos!, y: sliderLine.position.y + sliderHeight, width: verticalLineWidth, height: verticalBigLineHeight))
        }
    ...
    private func addVerticalLinesToLayer() {
        //need to call this to draw intital view
        updateLineHeight()
        //find out equal width
        let equalWidth = (sliderLine.frame.width-1)/9
        var finalValue = CGFloat(0)
        for n in 1 ... 8{
            if n == 1{
                finalValue = equalWidth + CGFloat(16)
                layer.addSublayer(drawVerticalLine(pos: finalValue,interval:n))
            }
            else{
                 finalValue = equalWidth + finalValue
               layer.addSublayer(drawVerticalLine(pos: finalValue,interval:n))
            }
        }
    }
